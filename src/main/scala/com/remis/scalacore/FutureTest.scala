  ...
  def main(args: Array[String]): Unit = {
    ...
    while (i < 10) {
      if (numreportsincurrbatch < 10) {
        val f = Future {
          val i2 = i; Thread.sleep(5000 + rnd.nextInt(5001)); println("Finished job " + i2); i2
        }
        futuresQ.enqueue(f)
        f.onComplete({
          case Success(result) => {
            numreportsincurrbatch -= 1; println("Success : " + result)
          }
          case Failure(t) => {
            println("An error has occurred: " + t.getMessage); numreportsincurrbatch -= 1;
          }
        })(executionContext)
